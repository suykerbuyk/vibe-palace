#!/usr/bin/env bash
#
# Orchestrator for `vp init` bash e2e cases.
#
# Contract:
...
#
# Usage:
#   bash test/e2e/init/run.sh                 # run all cases
#   bash test/e2e/init/run.sh 04              # run only 04-*.sh
#   bash test/e2e/init/run.sh 01 04 07        # run a subset
#
set -euo pipefail
...
TMPROOT="$(mktemp -d -t vp-e2e-init.XXXXXX)"
SUCCESS=0
