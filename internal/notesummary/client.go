...
// internal/enrichment/client.go's defaultEnrichTimeout and
// internal/itersummary/client.go's defaultGenerateTimeout.
const defaultGenerateTimeout = 10 * time.Second

// generate is the internal worker behind Summarizer.Generate. Mirrors
// internal/itersummary/client.go's generate exactly: returns (nil, nil) when
...
func generate(ctx context.Context, c llm.Completer, systemPrompt string, in PromptInput) (*Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("notesummary: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("notesummary reprompt: %w", err)
	}

	result, parseErr = parseResponse(retryContent)
	if parseErr != nil {
		return nil, fmt.Errorf("notesummary: %w", parseErr)
	}
	return result, nil
}
...
// Mirrors internal/enrichment/client.go's and internal/itersummary/client.go's
// unexported helpers of the same name (there is no shared/exported version to
...
// Summarizer is the LLM client for session-note summarization — NOT to be
// confused with internal/summarize.Summarizer, the queue-processing
// interface this package's SessionNoteSummarizer implements. Both are named
...
// Generate runs summarization for a single session note, bounded by the
