...
func TestEmbeddedCommands_CheckSuiteDelivery(t *testing.T) {
	...
	// The whole call, pinned as one literal — not just the eight names, and not
	...
	const selectors = `{"checks": ["vault-filesystem", "stray-scaffolds", "surface-merge-driver", "resume-caps", "resume-refs", "vault-abs-paths", "core-floor", "pin-coverage", "template-drift", "writer-identity", "stale-mcp"]}`
