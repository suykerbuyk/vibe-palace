...
func TestEmbeddedCommands_CheckSuiteDelivery(t *testing.T) {
	...
	const selectors = `{"checks": ["vault-filesystem", "stray-scaffolds", "surface-merge-driver", "resume-caps", "resume-refs", "vault-abs-paths", "template-drift", "writer-identity", "stale-mcp", "release-version"]}`
