...
const ServerInstructions = "Call vp_bootstrap_context with project=<slug> at session start (example: {\"project\":\"<slug>\"}). It returns an INDEX, not documents: head of queue, a session index, handles and instruments. The resume and workflow bodies are NOT in it — fetch them with vp_read_resource via resume_uri and workflow_uri on every restart. Prefer always naming project; on stdio MCP the server may derive it from a high-confidence cwd marker when omitted. When the user types vpc-<name> (e.g. vpc-restart, vpc-wrap), call " + agentfile.CommandToolName + " with name=<name> and follow the returned instructions. When the user types vps-<name>, call " + agentfile.SkillToolName + " with name=<name>. Call vp_capture_session at the end of each work unit. Large bodies may be delivered as vibe-palace:// resources — read them natively via resources/read or page them with vp_read_resource."
