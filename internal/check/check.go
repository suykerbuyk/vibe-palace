...
// classifier cannot judge adds a `<slug>: <err>` detail line. Always Info or
// Pass, never Fail — removal is a human decision. The .surface timestamp is
// surfaced as provenance so the operator can judge.
func CheckStrayScaffolds(v *storage.Vault) Result {
	...
	var stray, unclassified []string
	for _, e := range entries {
		name := e.Name()
		if !e.IsDir() || strings.HasPrefix(name, ".") || strings.HasPrefix(name, "_") {
			continue
		}
		state, cerr := storage.ClassifyProjectDir(v.Root, name)
		if cerr != nil {
			unclassified = append(unclassified, fmt.Sprintf("%s: %v", name, cerr))
			continue
		}
		if state != storage.ProjectScaffoldOnly {
			continue
		}
	...
	if len(stray) == 0 && len(unclassified) == 0 {
		r.Status = Pass
		r.Summary = "none"
		return r
	}
	...
	r.Status = Info
	switch {
	case len(stray) > 0 && len(unclassified) > 0:
		r.Summary = fmt.Sprintf("%d scaffold-only project(s) with no content; %d could not be classified",
			len(stray), len(unclassified))
	case len(stray) > 0:
		r.Summary = fmt.Sprintf("%d scaffold-only project(s) with no content", len(stray))
	default:
		r.Summary = fmt.Sprintf("%d project(s) could not be classified", len(unclassified))
	}
	if len(stray) > 0 {
	...
	r.Details = append(r.Details, unclassified...)
	return r
}
