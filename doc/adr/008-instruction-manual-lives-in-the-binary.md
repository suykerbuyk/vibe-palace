...
- **The served `workflow.md` is ~100% generic doctrine.** Section-by-section, almost
  nothing in it is specific to the vibe-palace project. The genuinely project-specific
  workflow content is actually misfiled in `resume.md`'s pinned *Behavioral Notes*.
...
- **That bloat is the token-budget breach.** The live bootstrap payload is ~8,063 tokens
  against its own 8,000 default; the ~7.8 KB of vault doctrine bloat *is* the overage. The
  payload rides ~63 tokens over its budget for this reason — though the
  `TestBootstrapLiveVaultFitsItsOwnBudget` canary as written currently *passes*, because the
  shed ladder measures the payload without the `Budget` field it attaches afterward and so
  reports `over=false` (see the measurement bug under Costs/risks).
...
- **Instruction propagation is a `config sync` tax with no project payoff.** Of the seven
  reconcilers `vp config sync` runs, exactly one — Templates/Materialize — exists solely to
  copy the 36-file embedded instruction corpus onto disk and track it in `templates.lock`.
  That churn (and its conflict class) is entirely about updating *instructions*, not
  project data.
...
**Costs / risks**
- The bootstrap "un-sheddable workflow contract" guard (`context_tools.go:775-788`) assumes
  the behavioral rules physically live in the inlined workflow body. When doctrine moves to
  the on-demand surface, that guard's semantics must move with it or it becomes dead weight.
- New surface area: a `vp_manual`/`vp_capabilities` tool and a `vp_knowledge_write` typed
  writer (a gap the audit exposed — `cancel-plan.md` currently edits `knowledge.md` via raw
  file tools). Both are read/typed-write additions; adding a read-only tool does not by
  itself require an `MCPSurfaceVersion` bump, but the tool-surface golden must be regenerated.
...
- A separate, real bug surfaced: the shed ladder measures the payload *without* the `Budget`
  field it attaches afterward, so it reports `over=false` while shipping ~63 tokens over.
  Fix it so the canary and the runtime agree.
...
- **Subsumes the planned "ADR-008 workflow shrink"** earmarked by
  `workflow-md-is-the-new-binding-constraint-on-the-payload`: shrinking `workflow.md` is a
  consequence of relocating the doctrine, not a separate pruning exercise.
...
- ADR-006 (Derive, Don't Ask); ADR-003 (vault-write locking).
- Tasks: `workflow-md-is-the-new-binding-constraint-on-the-payload`,
  `bootstrap-payload-exceeds-its-own-token-budget` (both subsumed/closed by this work),
  `mcp-served-doctrine-and-thin-project-workflow` (the implementation).
