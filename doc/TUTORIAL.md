...
> '^TestIntegrationE2EWalkthrough' -v ./internal/integration/...` from
> the repo root. The test prints an annotated transcript of the same
...
A persona is only ever adopted deliberately: the user types `/vps-<name>`
or `vps-<name>`. No surface adopts one because the conversation happened
to match what the skill is for. Each shim's description is a one-line
`Vibe-palace skill — …` label, never the skill's trigger text.
...
| Cursor                      | Native rule file (`.cursor/rules/vps-<name>.mdc`)           | Pick from Rules panel, or typed `vps-<name>` | Only emitted when `.cursor/` or `.cursor/rules/` exists in the project. vp registers no MCP server with Cursor; without one, the rule's fallback runs `vp skills show <name>` from the project directory. |
